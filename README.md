...
Read those files :
- ``shaders/includes/ocean.gdshader`` (the sum of N wave trains)
- ``shaders/includes/[shallow/deep]_water.gdshaderinc`` (the control fields)
- ``shaders/dynamic_wave_trains.gdshaderinc`` (how to render the ocean from a heightmap)
...
That is a ``depth_map.exr``, for the refraction effect. A phase ``exemplar.exr`` and a ``profile.exr`` to build wave trains. A ``HDRI.exr`` for the env-map and a ``foam.jpg`` to add visual effects.
...
In the ``textures/`` folder, an phase field exemplar and a profile map are provided.
This folder contains code that generate those two textures ; To do so, open the ``precomputation/precomputation.tscn`` scene and run the ``Compute phase exemplar`` and ``Compute profile map`` commands.


# Shader dependencies

```
ocean --- tiling_and_blending -- footprints --- transforms
	  +-- [shallow/deep]_water
	  +-- visual_effects
```
...
- visual_effects : contains phase dependant functions that adds sub-surface scattering and foam to the ocean. Those functions are called in ``OceanFragment``
